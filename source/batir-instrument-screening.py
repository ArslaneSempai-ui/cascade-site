...
CSS = '''
  :root{--nuit-a:#33191f;--nuit-b:#241217;--nuit-c:#180b0f;
    ...
    --rubis-titre:#7a1f2e;--rubis-vif:#d64a5c;--rubis-clair:#ffc2c9;
    --texte:"Literata",Georgia,serif;--mono:"Roboto Mono",ui-monospace,Menlo,monospace;
    --montee:cubic-bezier(.16,.84,.32,1)}
  *{box-sizing:border-box;margin:0}
  html{scroll-behavior:smooth;caret-color:var(--rubis-vif);
    scrollbar-color:var(--rubis-titre) var(--nuit-c)}
  @media (prefers-reduced-motion:reduce){html{scroll-behavior:auto}}
  body{background:var(--nuit-b);color:var(--sur);font-family:var(--texte);line-height:1.55}
  img{max-width:100%;display:block}
  ::selection{background:var(--rubis-vif);color:var(--nuit-c)}
  a{text-underline-offset:4px;color:inherit}
  .sr{position:absolute;width:1px;height:1px;overflow:hidden;clip-path:inset(50%)}
  :focus-visible{outline:3px solid var(--rubis-vif);outline-offset:3px;border-radius:2px}
  ...
  .marque small{font-weight:500;color:var(--rubis-clair)}
  ...
  .barre nav a:hover{color:var(--sur);text-decoration:underline;
    text-decoration-color:var(--rubis-vif);text-decoration-thickness:1.5px}
  ...
  .terminal{position:relative;z-index:3;
    ...
    border:1px solid color-mix(in srgb,var(--rubis-vif) 30%,transparent);border-radius:14px;
    box-shadow:0 60px 140px rgba(0,0,0,.7),0 0 0 1px rgba(0,0,0,.4),
      inset 0 1px 0 color-mix(in srgb,var(--rubis-clair) 22%,transparent);overflow:hidden}
  ...
  .caret{display:inline-block;width:7px;height:14px;vertical-align:-2px;background:var(--rubis-clair);
  ...
  .t-page-halo{position:absolute;left:50%;top:52%;width:min(1100px,92vw);height:min(700px,70vw);
    ...
    background:radial-gradient(50% 50% at 50% 50%,color-mix(in srgb,var(--rubis-vif) 12%,transparent),transparent 70%)}
  ...
  .tm-barre i:first-child{background:var(--rubis-vif);
    box-shadow:0 0 8px color-mix(in srgb,var(--rubis-vif) 70%,transparent)}
  ...
  .tm-l .ps{color:var(--rubis-vif)}
  .tm-sortie{color:var(--sur);margin:2px 0 14px;line-height:1.7}
  .tm-sortie b{color:var(--rubis-clair);font-weight:500}
  .tm-preuve{font-size:12.5px;color:var(--sur-pale);line-height:1.7}
  .tm-preuve .ok{color:var(--rubis-vif)}
  .tm-preuve .ko{color:#ff7b6e;font-weight:700}
  .regl{font-family:var(--mono);font-size:12.5px;background:transparent;
    border:1px solid color-mix(in srgb,var(--rubis-vif) 40%,transparent);
    border-radius:8px;padding:8px 13px;cursor:pointer;color:var(--rubis-clair)}
  .regl:hover{border-color:var(--rubis-vif)}
  .regl.actif{background:var(--rubis-vif);color:var(--nuit-c);border-color:var(--rubis-vif)}
  ...
  .cell:hover{background:color-mix(in srgb,var(--rubis-vif) 12%,transparent);
    box-shadow:inset 0 0 0 1px color-mix(in srgb,var(--rubis-vif) 55%,transparent)}
  .cell[aria-pressed="true"]{background:color-mix(in srgb,var(--rubis-vif) 18%,transparent);
    color:var(--rubis-clair);box-shadow:inset 0 0 0 1.5px var(--rubis-vif)}
  .c-rappel{font-size:13px}
  .c-fp{font-size:11px;color:var(--sur-pale)}
  .cell[aria-pressed="true"] .c-fp{color:var(--rubis-clair)}

  .b-curseur{flex:1;min-width:280px}
  input[type=range]{width:100%;accent-color:var(--rubis-vif)}
  .b-ligne{display:flex;gap:18px;align-items:center;flex-wrap:wrap;margin:4px 0 8px}
  .b-val{font-family:var(--mono);font-size:13px;color:var(--rubis-clair);min-width:64px}
  ...
  .clone{margin-top:18px;background:color-mix(in srgb,var(--nuit-c) 92%,#000);
    border:1px solid color-mix(in srgb,var(--rubis-vif) 25%,transparent);border-radius:12px;
    padding:18px 22px;font-family:var(--mono);font-size:13px;line-height:1.9;overflow-x:auto}
  .clone .ps{color:var(--rubis-vif)}
...
<style>{CSS}</style>
<header class="barre">
  <a class="marque" href="../index.html">CASCADE <small>&#183; Screening</small></a>
  <nav aria-label="Site">
