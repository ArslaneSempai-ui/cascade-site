/**
 * Les briques de l'instrument AMÉTHYSTE, calculées par L'OUTIL, jamais recopiées.
 *
 * Copie adaptée de l'extracteur bleu (même contrat, mêmes refus) ; la même règle
 * vaut : la cellule recommandée PEUT ÊTRE ABSENTE (aucune cellule ne tient le
 * plancher sur les cas écrits, et le relevé le dit) : ce n'est pas un refus, c'est
 * un fait à afficher — « none holds the floor », avec la plus forte borne.
 *
 * Ce script importe le code de cascade-scoring (le registre des sept facteurs, la grille de
 * seuils, l'intervalle de Wilson, le scellé) et lit son relevé public releve-public.json.
 * Il émet instrument-scoring-donnees.json : pour chaque facteur présent, la grille des
 * 51 seuils, rappel et fausses alertes avec n et bornes, sur les dossiers étiquetés ET la
...
const OUTIL = join(homedir(), "Documents", "cascade-scoring");
const ICI = fileURLToPath(new URL(".", import.meta.url));

const { scelleIntact } = await import(join(OUTIL, "src", "empreinte.ts"));
const { rate } = await import(join(OUTIL, "src", "interval.ts"));
const { SEUILS, PALIERS } = await import(join(OUTIL, "src", "facteur.ts"));
// registre() du violet rend les sept facteurs, sans poids à réchauffer
const { registre } = await import(join(OUTIL, "src", "facteurs", "index.ts"));
...
if (JSON.stringify(duRegistre) !== JSON.stringify(duReleve)) {
  throw new Error(`the registry's factors (${duRegistre.join(", ")}) are not the record's (${duReleve.join(", ")}): the page would describe a tool that no longer exists. Re-run npm run measure -- --yes-overwrite in the tool.`);
}
...
const retenue = celluleRecommandee(releve.authored.tables, rangs, PLANCHER);
/* Sur les cas écrits du bleu, AUCUNE cellule ne tient 0,90 à la borne basse : le relevé
   le dit, la page doit le dire, pas le taire ni planter. La plus forte borne accompagne
   l'absence, pour que la page nomme ce qui est atteignable. */
const plusForteBorne = Math.max(...Object.values(releve.authored.tables)
  .flatMap((g) => Object.values(g).map((c) => c.rappel.bas)));
const recommandee = retenue !== null
  ? { plancher: PLANCHER, ...retenue }
  : { plancher: PLANCHER, absente: true, plusForteBorne };
...
const sortie = {
  ...
  tables: releve.tables,
  authored: {
    nEscalated: releve.authored.nEscalated,
    nMaintained: releve.authored.nMaintained,
  ...
  synthetic: "absent" in releve.synthetic
    ...
    : { nEscalated: releve.synthetic.nEscalated, nMaintained: releve.synthetic.nMaintained, grille: releve.synthetic.tables },
  recommandee,
};

writeFileSync(join(ICI, "instrument-scoring-donnees.json"), JSON.stringify(sortie, null, 1) + "\n");
console.log(`instrument-scoring-donnees.json: ${duReleve.length} factor(s), ${SEUILS.length} thresholds, `
  + `${recomposees} witness cell(s) recomposed, seal ${releve.empreinte} (commit ${releve.commit}).`);
console.log(recommandee.absente
  ? `no cell holds recall >= ${PLANCHER} at the lower bound on the written cases (strongest bound ${(recommandee.plusForteBorne * 100).toFixed(0)} %): said, not silenced.`
  : `recommended under recall >= ${PLANCHER} (lower bound): ${recommandee.palier} at threshold ${recommandee.seuil.toFixed(2)}.`);
