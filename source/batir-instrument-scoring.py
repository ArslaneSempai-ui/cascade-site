...
De instrument-scoring-donnees.json, emis par extraire-instrument-scoring.mjs :
le releve public scelle de cascade-scoring, verifie puis recompose cellule par
cellule temoin avec le rate() de l'outil avant d'avoir le droit d'exister. Les
dossiers sont ECRITS PAR LE DEPOT (42 escalades, 42 maintenues), la moitie
...
l'historique de revues du lecteur, c'est l'outil, chez lui : le terminal du bas
...
if r.returncode != 0:
    sys.exit(f"extraire-instrument-scoring a refuse :\n{r.stderr}")
print(" ", r.stdout.strip())
D = json.loads((BASE / "instrument-scoring-donnees.json").read_text())
...
def table_html():
    ...
    return f'''<div class="t-scroll"><table class="grille">
      <caption class="sr">Pick a cell: each shows recall on confirmed escalations over false alerts on maintained look-alikes, at that factor and threshold</caption>
      <thead><tr><th scope="col">factor \\ threshold</th>{tetes}</tr></thead>
      <tbody>{lignes}{absents}</tbody></table></div>'''


CSS = '''
  :root{--nuit-a:#241a3a;--nuit-b:#1a1230;--nuit-c:#100b1f;
    --sur:#ede6f7;--sur-pale:#b9a9d4;
    --papier:#dbd7c5;--papier-haut:#e2ddcb;--encre:#1b1d18;--demi:#4a4739;--pale:#55523f;
    --filet-clair:#bab7a0;
    --accent-titre:#4b2a7a;--accent-vif:#9b6fe0;--accent-clair:#e2d3ff;
  ...
  *{box-sizing:border-box;margin:0}
  ...
  /* le poste : le robot amethyste penche derriere le bord du terminal, coupe net */
  ...
  .terminal::after{content:"";position:absolute;inset:0;pointer-events:none;border-radius:14px;
    background:repeating-linear-gradient(to bottom,rgba(226,211,255,.016) 0 1px,transparent 1px 3px)}
  ...
  .clone{max-width:62ch;background:linear-gradient(163deg,var(--nuit-a),var(--nuit-b));
    ...
    color:var(--sur);box-shadow:0 6px 18px rgba(26,18,48,.22)}
...
JS = '''
  ...
  /* LA MEME REGLE QUE L'OUTIL, reappliquee ici : borne basse du rappel au plancher,
     puis le moins de fausses alertes, puis le facteur le moins cher, puis le seuil haut. */
  ...
  (() => {
    ...
    const el = $("#tm-preuve");
    /* la cellule émise peut être ABSENTE (aucune ne tient le plancher : le relevé le dit) :
       le self-check vérifie alors que la règle relue rend null AUSSI : l'accord sur
       l'absence est aussi vérifiable que l'accord sur une cellule */
    const ok = D.recommandee.absente
      ? relu === null
      : relu !== null && relu.palier === D.recommandee.palier && relu.seuil === D.recommandee.seuil;
    if (ok) {
...
PHRASE_ABSENTS = (
    f"a factor absent from tonight&#8217;s registry ({ABSENTS}) is a named row, never a guessed column"
    if ABSENTS else
    "every factor of the registry is present tonight; an absent one would be a named row, "
...
<meta property="og:description" content="Every risk factor at every threshold on the sealed public record: recall against false alerts, intervals everywhere, the tool's own selection rule under your recall floor.">
<meta property="og:url" content="https://cascade-routing.com/scoring/instrument.html">
<meta name="twitter:card" content="summary_large_image">
<meta name="description" content="Every risk factor at every threshold on the sealed public record: recall against false alerts, intervals everywhere, the tool's own selection rule under your recall floor.">
  ...
  <div class="colonne">
    ...
    <ul>
      ...
      <li><b>Files the repository wrote.</b> The labelled half is authored: archetypes of risk
        ...
        turnover) and benign look-alikes (a local shop, a salaried resident, a domestic SME,
      ...
      <li><b>The declared tables, sealed in.</b> Every weight the factors read (the country
...
(BASE / "INSTRUMENT-SCORING.html").write_text(PAGE, encoding="utf-8")
print(f"INSTRUMENT-SCORING.html {len(PAGE) / 1e3:.0f} ko")
