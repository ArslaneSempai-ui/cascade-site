    ...
    for q in QUESTIONS:
        if not D["questions"][q]["present"]:
            lignes += (f"<tr class='absent'><th scope='row'>{q}</th>"
                       f"<td colspan='{len(ORDRE)}'>no public test set yet: nothing judged, and said rather than guessed</td></tr>")
            continue
        cells = "".join(
...
JS = '''
  ...
  (() => {
    ...
    for (const [q, d] of Object.entries(D.questions)) {
      if (!d.present) { lignes.push("<b>" + q + "</b>: no public test set yet \\u00b7 nothing judged"); continue; }
...
<style>{CSS}{CSS_NOIR}{CSS_ONYX}{CSS_BARRE_SITE}</style>
{barre_site(courant=OUTILS["dossier"]["page_hero"], sceau=D["provenance"]["empreinte"], racine="../")}
...
{pied_html(outil=OUTILS["dossier"], sceau=D["provenance"]["empreinte"], tests=n_tests(OUTILS["dossier"]))}
